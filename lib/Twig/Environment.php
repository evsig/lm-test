...
class Twig_Environment
{
    ...
    {
		// блок для целей мобильной версии
		if (substr($name, 0, 2) == 'p-') {
			$origName = $name;
			if (CommonController::getViewTypePage($name) == CommonController::VIEW_TYPE_PAGE_MOBILE) {
				$name = 'mobile/' . $name; // переключаем на мобильный шаблон
			}
			if (CommonController::issetMobileTpl($origName)) {
				header('Vary: Accept-Encoding, User-Agent'); // отдаем заголовок о наличии мобильного версии страницы
			}
		}
		// /блок для целей мобильной версии
